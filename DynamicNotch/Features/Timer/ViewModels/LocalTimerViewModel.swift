...
class LocalTimerViewModel: ObservableObject {
    ...
    /// yields a zero duration and no-ops via the `duration > 0` guard. `now` is the same clock the
    /// caller used for that check, so the guard and this conversion can never disagree.
    func start(endsAt: Date, now: Date = Date(), label: String? = nil) {
        start(duration: endsAt.timeIntervalSince(now), label: label)
    }
