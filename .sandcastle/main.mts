...
// This is an Xcode project: building and testing it needs macOS + Xcode, which
// no Linux container has. Agents therefore run directly on the host, isolated
// only by their git worktree.
const SANDBOX = noSandbox();

// Host runs get no `--dangerously-skip-permissions`, so without an explicit mode
// the agent stalls on the first tool prompt. `auto` keeps approvals AI-mediated.
const AGENT = () => sandcastle.claudeCode(MODEL, { permissionMode: "auto" });
...
async function runRoot(root: Root): Promise<string | null> {
  ...
  for (let iteration = 1; iteration <= config.agent.retryRounds; iteration++) {
    ...
    for (const [index, wave] of waves.entries()) {
      ...
      // Serial on purpose: agents run on the host, and the suite drives the real
      // screen and a singleton DynamicNotch.app. Concurrent leaves would fail
      // each other's verification.
      for (const leaf of wave) {
        try {
          if (await runLeaf(leaf, leafBranch(leaf.id))) {
            landed.add(leaf.id);
          } else {
            console.error(`[${id}]   ✗ ${leaf.id}: ran out of iterations`);
          }
        } catch (cause) {
          console.error(`[${id}]   ✗ ${leaf.id}: ${cause}`);
        }
      }
    }
...
// Roots stay independent, but run one after another — same shared-host reason as
// the leaves inside a wave.
const settled: PromiseSettledResult<string | null>[] = [];
for (const root of roots) {
  try {
    settled.push({ status: "fulfilled", value: await runRoot(root) });
  } catch (reason) {
